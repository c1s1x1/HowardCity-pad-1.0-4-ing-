...
@property (nonatomic,strong) NSMutableArray *TaskCellFrame;
...
-(NSMutableArray *)statuses{
    if (!_TaskCellFrame) {
        _TaskCellFrame = [NSMutableArray array];
    }
    
    return _TaskCellFrame;
}
...
- (void)viewDidLoad {
    ...
    self.tableView.descriptionText = @"您暂且没有发布过任务";
...
-(void)initDate{
    ...
    if (![HowardCityTool TimeOut:self]) {
        ...
        NSString *LoginUrl = [[NSString alloc]initWithFormat:@"%@action=getPublishTasks&access_token=%@",HCInterfacePrefix,self.account.access_token];
#pragma mark 拼接url
...
{
    return self.TaskCellFrame.count;
//    return 10;
}

-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    
    HCTaskCellFrame *cellFrame = self.TaskCellFrame[indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.TaskCellFrm = self.TaskCellFrame[indexPath.row];
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    return cell;
//    UITableViewCell *cell = [[UITableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"ID"];
//    cell.textLabel.text = @"发布任务";
//    return cell;
}
...
- (void)willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration {
    ...
    HCLog(@"屏幕转了");
    NSMutableArray *cellFrmsM = [NSMutableArray array];
    for (HCTaskCellFrame *object in self.TaskCellFrame) {
        HCTaskCellFrame *cellFrm = [[HCTaskCellFrame alloc] init];
