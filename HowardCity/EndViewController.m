...
@interface EndViewController ()<UITableViewDataSource,UITableViewDelegate>
...
- (void)viewDidLoad {
    ...
    self.tableView.descriptionText = @"您暂且没有已完成的任务";
...
-(void)initDate{
    if (![HowardCityTool TimeOut:self]) {
        ...
        NSString *LoginUrl = [[NSString alloc]initWithFormat:@"%@action=getFinishTasks&access_token=%@",HCInterfacePrefix,self.account.access_token];
#pragma mark 拼接url
        ...
        [manager GET:LoginUrl parameters:NULL progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            NSDictionary *arry = [responseObject objectForKey:@"FinishtaskList"];
