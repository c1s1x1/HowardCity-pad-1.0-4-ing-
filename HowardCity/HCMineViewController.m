...
@interface HCMineViewController () <UITableViewDelegate, UITableViewDataSource,UserInfoDelegate>
{
...
//    [self.navigationController.navigationBar setBarTintColor:HCColor(34, 47, 104, 1)];
...
//    };
    ...
    self.headerView.backgroundColor = HCColor(55, 106, 133, 1);
...
#pragma mark 拼接url
        ...
        AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
        NSString *utf = [LoginUrl stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
#pragma mark 判断返回参数
        [manager GET:utf parameters:NULL progress:^(NSProgress * _Nonnull downloadProgress) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        ...
        cell.backgroundColor = HCColor(55, 106, 133, 1);
    ...
    cell.backgroundColor = HCColor(55, 106, 133, 1);
    if (indexPath.row == 1) {
        cell.imageView.image = [UIImage imageNamed:@"职称"];// 图片
    }
    if (indexPath.row == 2) {
        cell.imageView.image = [UIImage imageNamed:@"邮箱"];// 图片
    }
    if (indexPath.row == 3) {
        cell.imageView.image = [UIImage imageNamed:@"电话"];// 图片
    }
    if (indexPath.row == 4) {
        cell.imageView.image = [UIImage imageNamed:@"qq"];// 图片
    }
    ...
    cell.textLabel.textColor = HCColor(255, 255, 255, 1);
    cell.detailTextLabel.text = values[indexPath.row -1];// 子文本
    cell.detailTextLabel.textColor = HCColor(255, 255, 255, 1);
